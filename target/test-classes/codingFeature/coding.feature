Feature: Coding
  ...
  @instructions
  Scenario: How does this file work?
    Given  Read Me
  ...
  Scenario: coding
    ...
    Given Sum of Numbers - Digits in a String with Letters Java150k -> 6
    Given Fibonacci
    ...
    Given Add the count to the end of each character in consecutive order aaccbbaaa -> a2c2b2a3
    Given Find the maximum consecutive repeating character in the String aattaabfff -> f

  @collection
  Scenario: Using Collection Framework (lIST, SET, MAP)

    Given Add the count to end of each letter aaabbbccc -> a3b3c3 using Maps *ONLY WORKS IF THEY ARE IN ORDER*
  ...
  @basicLambda
  Scenario:Basic Lambda Expressions
  ...
  @lambdaCollections
  Scenario: Lambda Expressions with Collections
